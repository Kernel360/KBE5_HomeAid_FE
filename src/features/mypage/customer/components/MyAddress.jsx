...
const MyAddress = ({ onBack, onAddAddress }) => {
    const addresses = [
        ...
        { id: 2, name: '회사', address: '서울시 강남구 석촌호수로 456', isDefault: false }
    ];
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            <header className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center">
                    ...
                    <h1 className="text-xl font-bold text-gray-900">주소 관리</h1>
                </div>
                        ...
                        <div key={addr.id} className="bg-white rounded-2xl p-6 shadow-sm">
                            <div className="flex items-start justify-between mb-2">
                                <div className="flex items-center">
                                    ...
                                    {addr.isDefault && (
                                        <span className="ml-2 px-2 py-1 bg-blue-100 text-blue-600 text-xs rounded-full">기본</span>
                                    )}
                                </div>
                                <button className="text-gray-400" onClick={() => setCurrentView('addressRegister')}>
